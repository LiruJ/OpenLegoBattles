...
namespace OpenLegoBattles.TilemapSystem
{
    ...
    {
        #region Operators
        public TileData this[Point position] => this[position.X, position.Y];
        ...
        public bool HasTreeAtPosition(Point position) => !IsPositionInRange(position) || this[position].HasTree;
        ...
        public bool IsPositionInRange(Point position) => position.X >= 0 && position.X < Width && position.Y >= 0 && position.Y < Height;

        public bool IsPositionInRange(int x, int y) => x >= 0 && x < Width && y >= 0 && y < Height;
        #endregion

        #region Set Functions
        public void SetTreeAtPosition(Point position, bool hasTree) => mapData[position.X, position.Y].HasTree = hasTree;
